...
async function getMultiple(page = 1){
  ...
  const rows = await db.query(
    ...
    FROM clientstatus JOIN client ON client.ClientStatusID = clientstatus.ClientStatusID WHERE clientstatus.ClientStatus = 'Inactive' LIMIT ${offset},${config.listPerPage}`
  );
