...
async function getMultiple(id, page = 1){
  ...
  const rows = await db.query(
    `SELECT trainer.TrainerLastName AS 'Trainer Last Name', trainer.TrainerFirstName AS 'Trainer First Name', client.ClientLastName AS 'Client Last Name', client.ClientFirstName AS 'Client First Name', trainingtype.TrainingDescription AS 'Training Description',
    ...
    WHERE trainer.TrainerID = ${id} LIMIT ${offset},${config.listPerPage}`
  );
